...
class RentalController extends BaseController
{
    ...
    public function createRideRental($request, $user)
    {
        ...
        $package_item = PackageItem::where('id', $request->package_item_id)
            ->where('status', 1)
            ->first();
        ...
        $package = PackageMaster::where('id', $package_item->package_id)
            ->where('status', 1)
            ->first();
        ...
        if (request()->has('promo_code') && $request->promo_code != '') {
            ...
            $promo_count = RequestModel::where('promo_id', $promocode_id)
            ...
            $promo_all_count = RequestModel::where('promo_id', $promocode_id)
                ->where('is_completed', 1)
                ->count();
        ...
        if ($request->has('is_later') && $request->is_later == '1') {
            ...
            $userAlreadyTrip = RequestModel::where('user_id', $user->id)
                ->where('is_later', 1)
                ->where('is_completed', '!=', 1)
            ...
            if (!is_null($userAlreadyTrip)) {
                return $this->sendError(
                    'You allowed to run only one Ride later ',
                    [],
                    403
                );
            }
        ...
        $request_params = [
            ...
            'requested_currency_symbol' =>
                $package->getCountry->currency_symbol,
                ...
                foreach ($drivers->data as $key => $driver) {
                    ...
                    if ($driverdet) {
                        ...
                        if ($driverdet->active && $metta == 0) {
                            ...
                            $selected_drivers[$nova]['driver_id'] =
                                $driverdet->id;
                            $selected_drivers[$nova]['active'] =
                                $nova == 0 ? 1 : 0;
                            $selected_drivers[$nova]['request_id'] =
                                $request_detail->id;
                            $selected_drivers[$nova]['assign_method'] = 1;
                            $selected_drivers[$nova]['created_at'] = date(
                                'Y-m-d H:i:s'
                            );
                            $selected_drivers[$nova]['updated_at'] = date(
                                'Y-m-d H:i:s'
                            );
                            $nova++;
                        }
