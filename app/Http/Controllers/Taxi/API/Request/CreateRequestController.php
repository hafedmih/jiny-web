    ...
    public function createRequest(CreateTripRequest $request)
    {
        ...
        try{
            ...
            if ($request->has('ride_type') && $request->ride_type == "RENTAL") {
                return (new RentalController())->createRideRental($request,$user);
            }

            if ($request->has('ride_type') && $request->ride_type == "OUTSTATION") {
                return (new OutstationCreateController())->rideOutstation($request,$user);
            }
            // find zone
            ...
            if ($request->has('payment_opt')){
                ...
                foreach($payment as $option){
                 ...
                 if($value == 0) return $this->sendError('Payment Option is not avaialble. Plesae select another option',[],403);
            }
            ...
            if($request->has('is_instant')){
                ...
                $userModel = User::where('phone_number', 'like', '%'.$request->phone_number)->role('user')->first(); 
                if($userModel) {
        ...
        //    dd($request_params);
            ...
            if($booking_for == 'OTHERS'){
                ...
                $request_details = RequestModel::where('request_number',$request_detail->request_number)->update(['others_user_id'  => $others_user->id]);
                ...
                foreach ($drivers->data as $key => $driver) {
                    ...
                    if(!is_null($driverdet)){
                        ...
                        if($driverdet->active && $metta == 0){
                            ...
                            $selected_drivers[$noval]["active"] = ($noval == 0 ? 1 : 0);
