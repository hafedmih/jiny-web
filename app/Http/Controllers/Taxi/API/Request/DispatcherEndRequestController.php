...
class DispatcherEndRequestController extends BaseController
{
    use CommanFunctions;
    public function endRequest(DriverEndRequest $request)
    {
        ...
        $user = User::find($request_detail->driver_id);
        ...
        if($request_detail->is_instant_trip != 1){
            if ($request_detail->user_id != null) {
                ...
                sendPush($title,$sub_title, $pushData, $userModel->device_info_hash, $userModel->mobile_application_type,0);
                    ...
                    $title = 'The trip end by admin';
                    $body = 'The trip end by admin';
                    $sub_title = 'The trip end by admin';
                
                sendPush($title,$sub_title, $pushData, $driverModel->device_info_hash, $driverModel->mobile_application_type,0);
            }
        }else{
            $userModel = User::find($request_detail->user_id);
            if(!$userModel){
